...
GBool PDFDoc::newOutlineLevel(Object *node, Catalog* catalog, PyObject* list, int level=1)
{
    Object *curr, *next;
    ...
    if (node->dictLookup("First", curr)->isDict()) {
        do {
            ...
            if(page_number < 1 or page_number >  catalog->getNumPages())
                ...
                PyDict_SetItemString(local_dic, "page_number", PyLong_FromLong(page_number));
...
PyObject* PDFDoc::getToc(){
    PyObject* childs = PyList_New(0);
    Object* outline = getCatalog()->getOutline();
    if (outline != NULL && outline->isDict())
    {
        newOutlineLevel(outline, getCatalog(), childs);
    }
    return childs;
}

PyObject* PDFDoc::getInfo()
{
    Object *info;
    Object *obj;
