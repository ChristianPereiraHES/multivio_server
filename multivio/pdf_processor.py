    ...
    def search(self, query, from_=None, to_=None, max_results=None, sort=None, context_size=None, angle=0):
        ...
        for np in xrange(from_, to_):
            ...
            for i in xrange(0, lq):
                ...
                self.logger.debug(
                    "pdf_processor: searching subquery: %s on page %s..." % (subquery, np))
                ...
                # store coordinates
                coords = (found, x1, y1, x2, y2) = text_page.findText(subquery, startAtTop,
                                                                      stopAtBottom, startAtLast, stopAtLast, caseSensitive, backward)
                ...
                if (not found):
                    self.logger.debug(
                        "pdf_processor: word %s not found on page %s, skipping" % (subquery, np))
                    break
                ...
                while found is True:
                    ...
                    self.logger.debug(
                        "pdf_processor: found word %s on page %s, building structure" % (subquery, np))
                    ...
                    coords = (found, x1, y1, x2, y2) = text_page.findText(
                        subquery, startAtTop, stopAtBottom, startAtLast, stopAtLast, caseSensitive, backward)
                    coords = coords[1:]  # don't keep found boolean

                # last subquery was found on the page, we can add the results to the global results list now
                if (i == (lq - 1)):
                    self.logger.debug(
                        "pdf_processor: word %s found on page %s, adding to global results" % (subquery, np))
